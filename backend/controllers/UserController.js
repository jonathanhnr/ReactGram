...
const register = async (req, res) => {
    ...
    if (!newUser) {
        res.status(422).json({errors: ["Houve um errro "]})
        return
    }
    res.status(201).json({
        _id: newUser._id, token: generateToken(newUser._id)
    })
...
const getUserById = async (req, res) => {
    ...
    try {
        ...
        //check if user exist
        if (!user) {
            res.status(404).json({errors: ["Usuario nao encontrado1"]})
            return
        }

        res.status(200).json(user)
    } catch (error) {
        res.status(404).json({errors: ["Usuario nao encontrado2"]})
        return
        console.log(error)
    }
}
